          ...
          <React.Fragment>
            <div className="cd-flex cd-justify-between cd-items-center">
              <p className="cd-text-lg cd-font-semibold cd-text-gray-800 dark:cd-text-gray-200">
                Usted tiene {events?.length} eventos en {months.length} meses
              </p>
            </div>
        ...
        {!isLoading && percentageQuery.data === 1 && (
          <section className="cd-mt-8">
            ...
            >
              ...
              <button
                type="submit"
                disabled={askMutation.isPending || !prompt}
                className="cd-w-[10%] cd-ml-4 cd-py-[0.5rem] cd-px-[1rem] cd-bg-violet-500 cd-text-white cd-rounded-md cd-shadow-lg hover:cd-bg-violet-600 disabled:cd-cursor-not-allowed disabled:cd-bg-gray-300"
              >
                Ask
              </button>
            </form>
